...
def test_parse_ldetect2_log_local_search_rows(tmp_path: Path) -> None:
    ...
    path.write_text(
        "\n".join(
            [
                ...
                "hdf5_read_calls=7 hdf5_segment_partition_reads=11 "
                "hdf5_segment_loci=13 "
                "nocache_vcf_query_count=2 nocache_vcf_query_bp=1001 "
                "nocache_vcf_records_seen=900 nocache_vcf_records_retained=700 "
                "nocache_vcf_records_skipped=200 "
                "nocache_cyvcf2_query_count=2 nocache_tabix_query_count=0 "
                "nocache_cyvcf2_fallback_count=0 "
                "nocache_vcf_fetch_seconds=0.210000 "
                "nocache_vcf_decode_seconds=0.220000 "
                "nocache_array_prep_seconds=0.230000 "
                "nocache_row_generation_seconds=0.240000 "
                "nocache_duplicate_fallback_seconds=0.250000 "
                "nocache_duplicate_fallback_partitions=1 "
                "nocache_dosage_cache_hits=0 nocache_dosage_cache_misses=0 "
                "nocache_dosage_cache_evictions=0 nocache_dosage_cache_bytes=4096 "
                "nocache_ld_compute_seconds=0.260000 nocache_tile_count=3 "
                "nocache_max_tile_snps=500 nocache_pair_candidates=10000 "
                "nocache_pairs_after_cutoff=7000",
    ...
    assert breakpoints[0]["hdf5_read_calls"] == "7"
    assert breakpoints[0]["hdf5_segment_partition_reads"] == "11"
    assert breakpoints[0]["hdf5_segment_loci"] == "13"
    assert breakpoints[0]["nocache_vcf_query_count"] == "2"
    assert breakpoints[0]["nocache_vcf_fetch_seconds"] == "0.210000"
    assert breakpoints[0]["nocache_row_generation_seconds"] == "0.240000"
    assert breakpoints[0]["nocache_duplicate_fallback_partitions"] == "1"
    assert breakpoints[0]["nocache_dosage_cache_bytes"] == "4096"
    assert breakpoints[0]["nocache_max_tile_snps"] == "500"
    assert breakpoints[0]["nocache_pair_candidates"] == "10000"
    ...
    assert by_chrom[0]["hdf5_read_calls"] == "7"
    assert by_chrom[0]["hdf5_segment_partition_reads"] == "11"
    assert by_chrom[0]["hdf5_segment_loci"] == "13"
    assert by_chrom[0]["nocache_vcf_query_count"] == "2"
    assert by_chrom[0]["nocache_vcf_fetch_seconds"] == "0.210000"
    assert by_chrom[0]["nocache_row_generation_seconds"] == "0.240000"
    assert by_chrom[0]["nocache_duplicate_fallback_partitions"] == "1"
    assert by_chrom[0]["nocache_dosage_cache_bytes"] == "4096"
    assert by_chrom[0]["nocache_max_tile_snps"] == "500"
    assert by_chrom[0]["nocache_pair_candidates"] == "10000"
