...
multi-mode `ldetect run` (baseline lzf vs a candidate such as zstd or
zstd_shrink32), which isn't available in a fresh checkout, so it is
...
def test_compare_compression_flags_numeric_divergence_as_a_real_bug(
    ...
    """Unlike the signal-cache diagnostic, numeric divergence against a
    lossless candidate (e.g. zstd) indicates a real bug, not expected
    floating-point noise from a different computation order. The script
    itself doesn't know that distinction -- it just reports what it finds,
    which is also exactly what a lossy candidate (e.g. zstd_shrink32) needs."""
    module = _load_compare_compression_module()
    loci = [200]
    ...
    row = _read_tsv_row(output)
    assert row["candidate_mode"] == "zstd_shrink32"
