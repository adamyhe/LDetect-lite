...
multi-mode `ldetect2 run` (baseline lzf vs zstd vs zstd_f32), which isn't
available in a fresh checkout, so it is unit-tested here against hand-built
fixtures shaped like real output instead.
"""
...
def test_compare_compression_reports_exact_match_and_size_reduction(
    ...
    candidate_vector = tmp_path / "zstd-vector.txt.gz"
    _write_vector(baseline_vector, rows)
    _write_vector(candidate_vector, rows)

    baseline_bpoints = tmp_path / "baseline-bpoints.json"
    candidate_bpoints = tmp_path / "zstd-bpoints.json"
    _write_breakpoints(baseline_bpoints, "fourier_ls", loci)
    _write_breakpoints(candidate_bpoints, "fourier_ls", loci)

    baseline_bed = tmp_path / "baseline.bed"
    candidate_bed = tmp_path / "zstd.bed"
    write_bed(name="22", loci=loci, snp_first=100, snp_last=300, output=baseline_bed)
    write_bed(name="22", loci=loci, snp_first=100, snp_last=300, output=candidate_bed)

    baseline_cov_dir = tmp_path / "baseline_cov"
    candidate_cov_dir = tmp_path / "zstd_cov"
    _write_covariance_dir(baseline_cov_dir, [1000, 1000])
    _write_covariance_dir(candidate_cov_dir, [400, 400])

    baseline_bench = tmp_path / "baseline.benchmark.tsv"
    candidate_bench = tmp_path / "zstd.benchmark.tsv"
    _write_benchmark(baseline_bench, seconds=100.0, max_rss=500.0)
    _write_benchmark(candidate_bench, seconds=90.0, max_rss=480.0)
    ...
    _run_compare(
        ...
        [
            ...
            "--candidate-covariance-dir",
            str(candidate_cov_dir),
            "--baseline-benchmark",
            str(baseline_bench),
            "--candidate-benchmark",
            str(candidate_bench),
    ...
    assert row["candidate_mode"] == "zstd"
    ...
    assert int(row["candidate_covariance_bytes"]) == 800
...
def test_compare_compression_flags_numeric_divergence(tmp_path: Path) -> None:
    """For a lossy candidate (e.g. zstd_f32), numeric divergence is expected
    and this is exactly what the comparison is meant to surface -- the
    script just reports what it finds, without judging whether it's a bug."""
    module = _load_compare_compression_module()
    loci = [200]

    baseline_vector = tmp_path / "baseline-vector.txt.gz"
    candidate_vector = tmp_path / "zstd_f32-vector.txt.gz"
    _write_vector(baseline_vector, [(100, 1.0), (200, 2.5)])
    _write_vector(candidate_vector, [(100, 1.0), (200, 2.500001)])

    baseline_bpoints = tmp_path / "baseline-bpoints.json"
    candidate_bpoints = tmp_path / "zstd_f32-bpoints.json"
    _write_breakpoints(baseline_bpoints, "fourier_ls", loci)
    _write_breakpoints(candidate_bpoints, "fourier_ls", loci)

    baseline_bed = tmp_path / "baseline.bed"
    candidate_bed = tmp_path / "zstd_f32.bed"
    write_bed(name="22", loci=loci, snp_first=100, snp_last=300, output=baseline_bed)
    write_bed(name="22", loci=loci, snp_first=100, snp_last=300, output=candidate_bed)

    baseline_cov_dir = tmp_path / "baseline_cov"
    candidate_cov_dir = tmp_path / "zstd_f32_cov"
    _write_covariance_dir(baseline_cov_dir, [1000])
    _write_covariance_dir(candidate_cov_dir, [1000])
    ...
    row = _read_tsv_row(output)
    assert row["candidate_mode"] == "zstd_f32"
...
def test_compare_compression_handles_empty_covariance_dir(tmp_path: Path) -> None:
    ...
    _write_vector(candidate_vector, [(100, 1.0)])
    ...
    assert int(row["candidate_covariance_bytes"]) == 0
