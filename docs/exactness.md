...
The breakpoint and BED exactness above uses `find-minima`'s `symmetric`
default. This toy fixture's reference breakpoints were generated under scipy
<1.1, where a requested periodic Hann window was silently computed as
symmetric (`notes/findings/ldetect-original-reproduction.md`); `symmetric` is
also the formula Berisa & Pickrell's own supplement specifies mathematically,
which is why it's the CLI default rather than a reproduction-only fallback.
The Snakefile and the function-level/legacy-pipeline benchmark scripts under
`examples/ldetect_example/scripts/` all pin `symmetric` explicitly anyway, so
the fixture stays reproducible even if the default ever changes again.
