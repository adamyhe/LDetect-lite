...
- `--filter-window {symmetric,scipy-periodic}` — Hanning window mode for breakpoint filtering. `symmetric` uses `np.hanning` and is the default: it's the formula Berisa & Pickrell (2016) specify in their own supplement (`sin²(πn/(N-1))`, the denominator-`N-1`, symmetric-not-periodic form), and it's required to reproduce populations whose published reference blocks were generated under scipy <1.1, where periodic odd-length windows were bit-identical to symmetric ones. `scipy-periodic` matches modern SciPy's periodic Hann window (`scipy.signal.get_window(..., fftbins=True)`); it's needed for modern-scipy-era reproductions (e.g. MacDonald et al. 2022). See `notes/findings/ldetect-original-reproduction.md`.
