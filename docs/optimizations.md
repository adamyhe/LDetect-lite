...
The supported window mode is `scipy-periodic`, which uses SciPy's periodic Hann weights and matches original ldetect's `scipy.signal.get_window(..., fftbins=True)` behavior. The older `symmetric`/`np.hanning` mode is deprecated and kept only for historical diagnostics; new production and reproduction work should use the default `scipy-periodic` path.
