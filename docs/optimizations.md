...
Within-convolution parallelism is available separately via `--filter-workers N`. This uses a Numba `prange` kernel over output rows for each direct convolution. To avoid nested parallelism, ldetect-lite disables candidate-width trackback threading whenever `filter_workers > 1`; in that mode the sequential exponential/binary width choices still benefit from multiple threads inside each convolution, while trackback evaluates candidate widths one at a time.
...
The default window mode is `scipy-periodic`, which uses SciPy's periodic Hann weights and matches modern `scipy.signal.get_window(..., fftbins=True)` behavior. `symmetric`/`np.hanning` is not merely a legacy fallback: it's required to reproduce populations whose published reference blocks were generated under scipy <1.1, where `get_window`'s periodic-window construction was a no-op for odd-length windows (the only length `2*width+1` ever produces) and therefore bit-identical to the symmetric window. See `notes/findings/ldetect-original-reproduction.md` for the full writeup.
...
Fixed at the Snakemake level, not inside the library: every rule that invokes `ldetect run` now exports `OMP_NUM_THREADS`/`OPENBLAS_NUM_THREADS`/`MKL_NUM_THREADS`/`NUMEXPR_NUM_THREADS`/`NUMBA_NUM_THREADS`, all set to `1`, before the command. That is the right default for this process-parallel pipeline: `--workers 4` should mean four worker processes total, not four worker processes that can each start four or more native BLAS/OpenMP/Numba threads. This is reliable because each Snakemake job is a fresh subprocess, so there's no import-ordering concern — the environment is fixed before the interpreter (and therefore numpy) even starts. `ldetect run` itself can't force this the same way: `_cli/main.py::main()` imports numpy-dependent submodules before `argparse` parses `--workers`, so by the time the CLI knows the worker count, BLAS may already have sized its thread pool from whatever was already in the environment. Silently mutating `os.environ` from inside a library function is also a footgun for anyone embedding `ldetect_lite` in a larger process. Added a startup warning instead (`_run` in `cmd_run.py`): if `--workers > 1` and any of the five variables are missing, `ldetect run` logs a warning so anyone invoking it directly (outside this Snakefile) gets nudged rather than silently oversubscribed. The warning doesn't fire for the standalone per-stage subcommands (`calc-covariance`, `matrix-to-vector`, `find-minima`) — only `run`.
