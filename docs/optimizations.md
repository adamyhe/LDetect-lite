...
**Exactness:** verified bit-exact against the `uint8` kernel in unit tests (`tests/test_shrinkage.py`) and at toy-example scale (`examples/ldetect_example/scripts/benchmark_functions.py --ld-kernel bitpacked`). The completed EUR 1000G serial diagnostic also produced exact downstream agreement between `uint8` and bitpacked (`vector_max_abs_diff=0.0`, exact loci/BED agreement). The remaining 1000G populations are being run before merge so the default flip is backed by population-wide validation rather than only EUR.

**Speed and memory:** preliminary serial EUR diagnostics show exact output with comparable RSS and a bitpacked advantage in the covariance row-generation kernel. The total pipeline speedup is smaller than the inner-kernel speedup because VCF/BCF ingestion and HDF5 writes remain substantial. Treat speed claims as preliminary until the remaining populations complete under the same benchmark conditions.
