...
Within-convolution parallelism is available via a split policy. Adaptive single-candidate phases (exponential search, binary search, and final minima extraction) use a Numba `prange` kernel over output rows with `max(--workers, --filter-workers)` threads available. Trackback keeps candidate-width threading because it has a small explicit candidate set per scan window, but forces each concurrent candidate filter to one Numba thread to avoid nested parallelism.
