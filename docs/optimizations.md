...
**Exactness:** verified bit-exact against the `uint8` kernel at both toy scale (`tests/test_shrinkage.py`, `examples/ldetect_example/scripts/benchmark_functions.py --ld-kernel bitpacked`) and full genome scale — `examples/ldetect_original/Snakefile.ld_kernel_diagnostics` compared all 22 chromosomes x 3 populations (EUR/AFR/ASN) on real 1000G data: every one of 66 chromosome x population combinations produced byte-identical covariance vectors, breakpoints, and BEDs (`vector_max_abs_diff=0.0`, `bed_jaccard=1.0` throughout). Peak RSS was also comparable to `uint8` (ratio 0.93-1.22 across chromosomes, no systematic regression).

**Speed:** not yet established. The same genome-scale diagnostic's wall-clock numbers were too noisy to trust — extreme outliers (0.12x-11.27x per-chromosome) concentrated in the shortest-running jobs, consistent with fixed per-run overhead or scheduling contention (see #13) rather than a real kernel-speed difference. `uint8` remains the default; see `notes/logs/bitpacked-ld-kernel.md` for the full validation history and what a trustworthy speed measurement would need (the #13 thread-count-guard treatment, or an isolated run).
