...
1. **Genetic-map deserts** (Category A — chr9, and 7 other chromosome/population combinations). Some chromosomes have large (tens-of-Mb) genetic-map dead zones (e.g. chr9's 17 Mb desert, consistent with its heterochromatic 9qh block). Legacy LDetect emits one unsplit block across the whole desert; `ldetect2` places an extra breakpoint inside it (same total genome-wide breakpoint budget, different placement) — confirmed directly against MacDonald's own git history (raw pre-centromere-removal BEDs).
