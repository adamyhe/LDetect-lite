...
- `--covariance-mode {partition,chromosome}` — covariance calculation strategy (default: `partition`). `partition` reads each partition region independently and can parallelize partitions with `--workers`; `chromosome` loads the chromosome genotypes once, slices each partition from prepared arrays, and currently requires `--covariance-cache compact`.
...
- `--ld-kernel {uint8,bitpacked}` — compact covariance pair-count backend (default: `uint8`). `bitpacked` uses packed haplotypes and popcounts and requires compact covariance output.
- `--profile-covariance PATH` — write covariance timing diagnostics as TSV. In chromosome mode this includes one chromosome-load row plus per-partition writer rows.
