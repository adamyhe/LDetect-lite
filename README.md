...
If `--workers` is greater than 1 and any of `OMP_NUM_THREADS`/`OPENBLAS_NUM_THREADS`/`MKL_NUM_THREADS`/`NUMEXPR_NUM_THREADS`/`NUMBA_NUM_THREADS` are missing, `run` prints a startup warning: numpy/BLAS/numba otherwise may size uncapped internal thread pools to the whole machine's core count, which can oversubscribe real CPUs when process-pool workers or multiple `ldetect run` jobs share a node. For the default process-parallel pipeline, export those five variables to `1` unless you are deliberately using an intra-operation threading option such as `--filter-workers` — see `examples/ldetect_original/Snakefile`'s `run_ldetect` rule for a worked example.
