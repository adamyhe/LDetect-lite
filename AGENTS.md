...
Tracked in detail in `notes/ldetect-original-main-pipeline-audit.md` (log of record) and `notes/ldetect-original-handoff.md` (current-state snapshot — start here). As of the latest full genome-wide run against the published Bitbucket `ldetect-data` reference BEDs:
