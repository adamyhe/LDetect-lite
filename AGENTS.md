...
| `filters.py`                  | Hanning-window convolution (`scipy-periodic` by default; deprecated `symmetric`/`np.hanning` mode retained for diagnostics) and `scipy.signal.argrelextrema` minima extraction                                                                                                                                                                                                                                                      |
...
- **SciPy-periodic Hann window**: `filters.py` defaults to `scipy.signal.get_window('hann', N)` (`fftbins=True`), matching original ldetect's periodic Hann behavior. The older `symmetric`/`np.hanning(2*width+1)` mode is deprecated and retained only for historical diagnostic comparisons; do not make it the production or reproduction default.
