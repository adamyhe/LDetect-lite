...
| `ld_neighborhood.py`          | Left/across/right LD-separation sanity check: `chromosome_separation_samples` reservoir-samples r² around every computed breakpoint from the covariance cache, `write_separation_boxplot` renders it. Self-contained (no reference BED needed) — wired into `ldetect run --generate-ld-neighborhood-plot`. Ported from `examples/MacDonald2022/scripts/plot_ld_neighborhood_separation.py`, which still runs a per-chromosome/genome-wide version comparing against a reference in the reproduction Snakefiles. |
