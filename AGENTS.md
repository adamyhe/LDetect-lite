...
| `filters.py`                  | Hanning-window convolution (`scipy-periodic` by default; `symmetric`/`np.hanning` mode required to reproduce populations whose reference blocks predate scipy 1.1, see below) and `scipy.signal.argrelextrema` minima extraction                                                                                                                                                                                                                                                      |
...
- **SciPy-periodic Hann window**: `filters.py` defaults to `scipy.signal.get_window('hann', N)` (`fftbins=True`), matching modern scipy's periodic Hann behavior. `symmetric`/`np.hanning(2*width+1)` is not a legacy fallback — it's required for `ldetect_original`, whose Berisa & Pickrell reference blocks were generated under scipy <1.1, where `get_window`'s periodic-window construction was a no-op for odd-length windows (the only length `2*width+1` produces) and therefore bit-identical to the symmetric window. That bug was fixed in scipy 1.1.0 (2018); reproductions run against modern-scipy-era published output (e.g. MacDonald2022) correctly need `scipy-periodic` instead. See `notes/findings/ldetect-original-reproduction.md`.
