...
| `filters.py`                  | Hanning-window convolution (`symmetric`/`np.hanning` by default, matching the formula Berisa & Pickrell's supplement specifies; `scipy-periodic` mode required to reproduce modern-scipy-era populations, see below) and `scipy.signal.argrelextrema` minima extraction                                                                                                                                                                                                                                                      |
...
| `ld_neighborhood.py`          | Left/across/right LD-separation sanity check: `chromosome_separation_samples` reservoir-samples r² around every computed breakpoint from the covariance cache, `write_separation_boxplot` renders it. Self-contained (no reference BED needed) — wired into `ldetect run --generate-ld-neighborhood-plot`. Its low-level pieces (`category_masks`, `read_diagonal_index`, `r2_for_pairs`, `owned_bounds`, `reservoir_extend`) are shared with `examples/MacDonald2022/scripts/plot_ld_neighborhood_separation.py`, which imports them and adds its own per-boundary TSV output and support for sampling around an arbitrary reference BED's boundaries (used by the reproduction Snakefiles' "published" comparison rules) — capabilities the core module intentionally doesn't need. |
...
- **Symmetric Hann window default**: `filters.py` defaults to `np.hanning(2*width+1)` (the `symmetric` mode). This is not just a legacy-reproduction fallback — Berisa & Pickrell's own supplement specifies the window as `sin²(πn/(N-1))`, the denominator-`N-1` formula that is definitionally the symmetric (not periodic) Hann window, and `symmetric` is also the conventional windowed-FIR/convolution filter kernel choice. It's also required for `ldetect_original`, whose Berisa & Pickrell reference blocks were generated under scipy <1.1, where `get_window`'s periodic-window construction was a no-op for odd-length windows (the only length `2*width+1` produces) and therefore bit-identical to the symmetric window despite the original code literally requesting periodic (`scipy.signal.get_window('hann', N)`, `fftbins=True`). That bug was fixed in scipy 1.1.0 (2018); reproductions run against modern-scipy-era published output (e.g. MacDonald2022) correctly need `scipy-periodic` instead. See `notes/findings/ldetect-original-reproduction.md`.
