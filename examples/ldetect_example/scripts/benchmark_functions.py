...
def main() -> None:
    ...
    validate_ld_kernel(cfg.ld_kernel)
    require_inputs(cfg)
...
def validate_ld_kernel(ld_kernel: str) -> None:
    params = inspect.signature(calc_covariance).parameters
    if ld_kernel != "uint8" and "ld_kernel" not in params:
        raise SystemExit(
            "--ld-kernel bitpacked requires a newer ldetect-lite checkout with "
            "bitpacked calc_covariance support"
        )
...
def run_pipeline(
    ...
    time_stage(
        ...
        lambda: calc_covariance(
            **calc_covariance_kwargs(cfg, covariance),
        ),
    )
    time_stage(
        "matrix_to_vector",
...
def calc_covariance_kwargs(cfg: Config, output_path: Path) -> dict:
    kwargs = {
    ...
    if "ld_kernel" in inspect.signature(calc_covariance).parameters:
        kwargs["ld_kernel"] = cfg.ld_kernel
    return kwargs
...
def repeat_colors(n: int) -> list[str]:
    palette = ["#0057b8", "#d62728", "#009e73", "#e69f00", "#6b6b6b"]
