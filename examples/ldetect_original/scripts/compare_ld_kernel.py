...
Both modes run on identical filtered VCF/map/individuals input with
`--covariance-mode partition` (chromosome mode is out of scope for this
diagnostic). The bitpacked kernel computes the same popcount-derived pair
counts as the uint8 kernel and feeds them through the same shrinkage
formula, so the two outputs are expected to be exact matches, not merely
close. This script reports:
