"""Compare a candidate `ldetect run` covariance-storage mode against `baseline`.

Both modes run on identical filtered VCF/map/individuals input. `zstd`
(`--covariance-compression zstd`) is a lossless codec swap, so its outputs are
expected to be exact matches against `baseline` (`--covariance-compression
lzf`) -- the codec only changes how covariance rows are stored on disk, not
the values themselves. `zstd_shrink32` (`--covariance-compression zstd
--shrink-ld-precision float32`) additionally stores `shrink_ld`/`diag_val` as
real on-disk float32 -- this is lossy, so exact matches are NOT guaranteed;
measuring whether that precision loss ever shifts a real breakpoint is exactly
what this diagnostic is for. This script reports:
...
Usage:
    uv run python scripts/compare_compression.py \
        --population EUR --chromosome 22 --candidate-mode zstd_shrink32 \
        --baseline-vector .../baseline/vector-22.txt.gz \
        --candidate-vector .../zstd_shrink32/vector-22.txt.gz \
        --baseline-breakpoints .../baseline/breakpoints-22.json \
        --candidate-breakpoints .../zstd_shrink32/breakpoints-22.json \
        --baseline-bed .../baseline/22-ld-blocks.bed \
        --candidate-bed .../zstd_shrink32/22-ld-blocks.bed \
        --baseline-covariance-dir .../baseline/22 \
        --candidate-covariance-dir .../zstd_shrink32/22 \
        --baseline-benchmark .../logs/baseline.benchmark.tsv \
        --candidate-benchmark .../logs/zstd_shrink32.benchmark.tsv \
        --output results/.../compare/compression_vs_baseline.zstd_shrink32.tsv
"""
...
def main() -> None:
    ...
    parser.add_argument(
        ...
        required=True,
        help=(
            "Label for the non-baseline mode being compared "
            "(e.g. zstd, zstd_shrink32)."
        ),
    )
